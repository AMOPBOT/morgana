...
@user_admin_no_reply
@gloggable
def yonerm(update: Update, context: CallbackContext) -> str:
...
@user_admin_no_reply
@gloggable
def yoneadd(update: Update, context: CallbackContext) -> str:
    query: Optional[CallbackQuery] = update.callback_query
    user: Optional[User] = update.effective_user
...
def yone_message(context: CallbackContext, message):
...
CHATBOTK_HANDLER = CommandHandler("ChatBot", yone, run_async=True)
ADD_CHAT_HANDLER = CallbackQueryHandler(yoneadd, pattern=r"add_chat", run_async=True)
RM_CHAT_HANDLER = CallbackQueryHandler(yonerm, pattern=r"rm_chat", run_async=True)
